...
function removeFirstOccurrences(str, value) {
  ...
  const endSubstr = startSubstr + value.length;
  if (startSubstr > -1) {
    return str.slice(0, startSubstr) + str.slice(endSubstr);
  }
  return str;
}
...
function removeLastOccurrences(str, value) {
  const startSubstr = str.lastIndexOf(value);
  const endSubstr = startSubstr + value.length;
  if (startSubstr > -1) {
    return str.slice(0, startSubstr) + str.slice(endSubstr);
  }
...
function formatTime(minutes, seconds) {
  const getNumberFormated = (num) => num.toString().padStart(2, '0');
  return `${getNumberFormated(minutes)}:${getNumberFormated(seconds)}`;
}
...
function orderAlphabetically(str) {
  const arrLetter = [...str];
  const sortArr = arrLetter.sort((a, b) => a.localeCompare(b));
  return sortArr.join('');
}
...
function extractNameFromTemplate(value) {
  ...
  return `${arrWords[1]} ${arrWords[2].slice(0, -1)}`;
}
...
function encodeToRot13(str) {
  ...
  const arrTableLookup = [[...tablesLookup[0]], [...tablesLookup[1]]];

  const res = [...str].map((item) =>
    tablesLookup[0].includes(item)
      ? arrTableLookup[1][arrTableLookup[0].indexOf(item)]
      : item
  );
